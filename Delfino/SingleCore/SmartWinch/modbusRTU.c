...
int receive_request(unsigned char *received_string) 
{
        ...
        while (SCI_getRxFIFOStatus(SCIA_BASE)) {
                ...
                received_string[bytes_received] = (unsigned char) SCI_readCharNonBlocking(SCIA_BASE);
...
uint32_t T35_char = 16U; //this should give around 3.5 char long for 200us per tick @ 115200bps
...
unsigned int regs_size) 
{
        ...
        unsigned int start_addr;
        int exception;
        int length = SCI_getRxFIFOStatus(SCIA_BASE);
        uint32_t now = CPUTimer_getTimerCount(CPUTIMER0_BASE);
        volatile static uint32_t Nowdt = 0x00;
