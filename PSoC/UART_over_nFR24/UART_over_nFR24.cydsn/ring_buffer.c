...
uint16_t circular_buf_available(circular_buf_t cbuf)
{
    uint16_t free;
    
    if(cbuf.head >= cbuf.tail)
        free = cbuf.size - (cbuf.head - cbuf.tail);
    else
        free = cbuf.tail - cbuf.head - 1;
    
    return  (cbuf.size - free);
}
...
{
    return circular_buf_available(cbuf);
}
