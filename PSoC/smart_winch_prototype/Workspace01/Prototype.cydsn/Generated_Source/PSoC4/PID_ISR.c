...
{
    CyRamVectors[CYINT_IRQ_BASE + PID_ISR__INTC_NUMBER] = address;
}
...
{
    return CyRamVectors[CYINT_IRQ_BASE + PID_ISR__INTC_NUMBER];
}
...
void PID_ISR_SetPriority(uint8 priority)
{
    ...
    uint32 priorityOffset = ((PID_ISR__INTC_NUMBER % 4u) * 8u) + 6u;
    
	interruptState = CyEnterCriticalSection();
    *PID_ISR_INTC_PRIOR = (*PID_ISR_INTC_PRIOR & (uint32)(~PID_ISR__INTC_PRIOR_MASK)) |
...
uint8 PID_ISR_GetPriority(void)
{
    ...
    priority = (*PID_ISR_INTC_PRIOR & PID_ISR__INTC_PRIOR_MASK) >> priorityOffset;
